...
// Write a function to compute the force between two particles:
void compute_force(Particle *p1, Particle *p2, double *fx, double *fy, double *fz) {
    ...
    double eps = 1e-3; // Small constant to avoid division by zero
...
// Write a function to initialize the particles:
...
int main(int argc, char *argv[]) {
    int num_particles = 50000; // set total number of particles
...
for (int step = 0; step < num_steps; step++) {
    ...
    // Only the root process should print the time.
    if (rank == 0) {
